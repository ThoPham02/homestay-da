...
export const authService = {
  ...
  async login(credentials: LoginRequest): Promise<LoginResponse> {
    try {
      ...
      throw new Error(error.response?.data?.result?.message || error.response?.data?.message || 'Đăng nhập thất bại');
    }
  },
  ...
  async register(userData: RegisterRequest): Promise<LoginResponse> {
    try {
      ...
      toastService.showApiError(error);
      throw new Error(error.response?.data?.result?.message || error.response?.data?.message || 'Đăng ký thất bại');
    }
  },
  ...
  async getProfile(): Promise<ProfileResponse> {
    try {
      ...
      toastService.showApiError(error);
      throw new Error(error.response?.data?.result?.message || error.response?.data?.message || 'Không thể lấy thông tin profile');
    }
  },
